Function Add-SqlServerUserToRole {
    ...
    param(
        [Parameter(Position = 0, ValueFromPipeline)][string] $server,
        [Parameter(Position = 1, ValueFromPipelineByPropertyName)][string] $database,
        [Parameter(Position = 2, ValueFromPipelineByPropertyName)][string] $user,
        [Parameter(Position = 3, ValueFromPipelineByPropertyName)][string] $role
    )
