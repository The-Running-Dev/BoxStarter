...
$installVersionDir  = Join-Path $installDir  "DataGrip 2016.2.3"

if (Test-Path ($installVersionDir)) {

  $uninstallExe = (gci "${installDir}/WebStorm 2016.2.3/bin/Uninstall.exe").FullName | sort -Descending | Select -first 1
